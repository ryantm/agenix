...
/// Handles Nix expression evaluation for agenix rules
#[derive(Debug)]
...
impl NixManager {
    ...
    pub fn get_public_keys(&self, file: &str) -> Result<Vec<String>> {
        let nix_expr = format!(
            "(let rules = import {}; in rules.\"{}\".publicKeys)",
            self.config.rules_path, file
        );

        let output = Command::new(&self.config.nix_instantiate)
        ...
        if !output.status.success() {
            ...
            return Err(anyhow!("Failed to get keys for file: {}, nix error: {}", file, stderr));
        }

        let json_str = String::from_utf8(output.stdout)
            .context("Failed to parse nix output as UTF-8")?;
        
        let json_value: Value = serde_json::from_str(&json_str)
            .context("Failed to parse nix output as JSON")?;
        ...
        let keys = match json_value {
            ...
            _ => return Err(anyhow!("Expected JSON array for public keys, got: {}", json_value))
        };
    ...
    pub fn should_armor(&self, file: &str) -> Result<bool> {
        let nix_expr = format!(
            "(let rules = import {}; in (builtins.hasAttr \"armor\" rules.\"{}\" && rules.\"{}\".armor))",
            self.config.rules_path, file, file
        );
    ...
    pub fn get_all_files(&self) -> Result<Vec<String>> {
        ...
        let files = match json_value {
            ...
            _ => return Err(anyhow!("Expected JSON array for file names, got: {}", json_value))
        };
    ...
    pub fn validate_rules_file(&self) -> Result<()> {
        if !std::path::Path::new(&self.config.rules_path).exists() {
            return Err(anyhow!(
    ...
    pub fn eval_json(&self, expr: &str) -> Result<Value> {
        let output = Command::new(&self.config.nix_instantiate)
...
mod tests {
  ...
  "secret2.age" = {{
    ...
    #[test]
    fn test_nix_manager_creation() {
        let config = create_test_config();
        let manager = NixManager::new(config);
    ...
    fn test_validate_rules_file_nonexistent() {
        ...
        let result = manager.validate_rules_file();
    ...
    fn test_eval_json_simple() {
        ...
        if let Ok(result) = manager.eval_json("42") {
    ...
    fn test_eval_json_invalid() {
        ...
        let result = manager.eval_json("invalid nix expression !!!");
    ...
    mod integration_tests {
        ...
        fn test_get_public_keys_with_real_nix() -> Result<()> {
            ...
            // and nix-instantiate + jq available
            let result = manager.get_public_keys("secret1.age");
        ...
        fn test_should_armor_with_real_nix() -> Result<()> {
            ...
            match manager.should_armor("secret1.age") {
            ...
            match manager.should_armor("secret2.age") {
